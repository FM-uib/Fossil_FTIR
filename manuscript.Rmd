---
title: 'Assessing variations in the chemistry of sub-fossil and modern *Pinus* pollen'
#date: '2020/02/04'
output:
...
The study of pollen-grain chemistry has gained increasing focus in palaeoecological research since it is possible that chemical variations observed in sporopollenin may be used for identification of pollen grains.
Fourier Transform Infrared (FTIR) spectroscopy has become one of the most popular methods for characterising the chemical variations present within pollen and spores.
...
We also investigate the effect of two pollen extraction methods (acetolysis and density separation) on fossil pollen chemistry.
Our methods reproduce modern *Pinus* spectra well, compared to bulk analysis and other microspectroscopy approaches.
We report chemical differences between fossil and modern pollen, mainly lack of lipid and protein wavebands (1800 - 1500 cm^-1^), in addition to changes in the carbohydrate region (1200 - 1000 cm^-2^). 
...
We also report contaminations observed in non-acetolysed samples and the applications of high density separation extraction for FTIR microspectroscopy and palynology.
...
Bulk FTIR techniques, which involve the analysis of large quantities of modern-pollen samples (approximately 1-2 mg) (e.g. @zimmermannCharacterizationPollenVibrational2010, @lahlaliATRFTIRSpectroscopyReveals2014), predominantly result with high-quality spectra with detailed biochemical ‘fingerprint’ of pollen, comprising specific signals of sporopollenins, lipids, proteins, carbohydrates and other chemical constituents [@bagciogluMultiscaleVibrationalSpectroscopic2015].
As a result, FTIR spectra obtained from bulk pollen samples have been used to identify chemical differences at the genus, and in some cases, species and population level [@zimmermannInfraredSpectroscopyPollen2014; @bagciogluMonitoringPlantEnvironment2017; @zimmermannHighthroughputFTIRSpectroscopy2017; @depciuchIdentificationBirchPollen2018; @muthreichChemicalVariationsQuercus2020; @diehnDiscriminationGrassPollen2020].
...
An alternative approach involves the use of FTIR microscopes, where a FTIR spectrometer is coupled to a microscope. 
This enables acquisition of infrared spectra from a small area, with aperture covering several pollen grains, a single pollen grain, or even pollen grain substructure [@zimmermannVibrationalMicrospectroscopyEnables2015].
FTIR-microscopes are better suited to sediment samples as a result of the capacity to generate FTIR spectra from individual grains.
However, FTIR microspectroscopy is hindered by strong Mie scattering of IR radiation on pollen grains, resulting with anomalous features in their FTIR spectra [@lukacsRecoveryAbsorbanceSpectra2015; @zimmermannVibrationalMicrospectroscopyEnables2015; @blumelObservationMieRipples2018a].
Although the scattering issues can be tackled by a number of analytical and numerical solutions [@lukacsRecoveryAbsorbanceSpectra2015; @zimmermannAnalysisAllergenicPollen2016; @zimmermannChemicalCharacterizationIdentification2018; @diehnDiscriminationGrassPollen2020], FTIR measurement of individual pollen grains remains challenging.
...
Some FTIR microscopes are equipped with automated sample stages, enabling automated measurement of predetermined measurement positions.
Measurements are usually acquired by consecutive recording of spectra by a single element detector [@zimmermannVibrationalMicrospectroscopyEnables2015; @zimmermannAnalysisAllergenicPollen2016; @zimmermannChemicalCharacterizationIdentification2018].
When spectra are recorded with equidistant spatial steps (i.e. scanning measurement), the individual spectra can be assembled after the recording to obtain a chemical image [@zimmermannVibrationalMicrospectroscopyEnables2015; @julierChemotaxonomyToolInterpreting2016; @jardineSporopolleninChemistryIts2021].
However, recent developments in technologies related to FTIR allow for easier and faster capture of spectra of pollen grains. 
A focal plane array detector (FPA) is made of a 2D array of IR detectors, and allows true chemical imaging.
This enables simultaneous measurement of several single pollen grains from a relatively large surface area without the need to move the sample stage.
Therefore, it is of potentially high relevance for palaeoecological research. 

When combined with imaging processing technology, it may allow for more rapid identification and extraction of FTIR spectra of pollen grains from sediment samples.
Whilst FTIR microspectroscopy with FPA detectors has been widely applied with microplastics [@primpkeAutomatedApproachMicroplastics2017] application on pollen samples was limited just to chemical characterization of isolated sporopollenin capsules [@chiappePollenGrainsFunctionalized2017].
In particular, there has been no investigation into application of FPA detectors on measurement of fossil-pollen residues taken from sediment cores.
...
One of the most common methods for sporopollenin extraction involves an acetolysis treatment, which, alongside a combination of treatments with hydrofluoric acid (HF) and hydrochloric acid (HCl), enables removal of organic materials, calcium carbonates and silicates, while preserving the morphological integrity of pollen grains [@erdtmanSixNotesPollen1959; @faegriTextbookPollenAnalysis1989].
Acetolysis has also often been used in combination with FTIR on pollen as a way to standardise between modern and fossil samples, [@jardinePollenSporesBiological2016; @woutersenNovelApproachStudy2018] or to study the effects of acid extractions [@schulzeosthoffPhenolsIntegratedCompounds1987; @dominguezIsolationIntactPollen1998; @dominguezPollenSporopolleninDegradation1999; @loaderPreparationPollenStable2000;  @jardineImpactOxidationSpore2015; @jardineSporopolleninChemistryIts2021].
...
Given the large variations in FTIR spectra observed across taxa [@diehnCombiningChemicalInformation2020; @muthreichChemicalVariationsQuercus2020], and the variety of sediment types even from Holocene sequences in Europe, more studies are needed to investigate the potential effects of the chemical preparation procedures used to isolate pollen grains from different pollen types.

In this study we aimed to explore the use of a FTIR imaging to obtain IR spectra from fossil-pollen *Pinus* grains.
We develop a workflow which enables comparison of fossil-pollen grains taken from three sediment-core locations (Table \@ref(tab:sites)), and compare the IR spectra from both modern and fossil pollen samples.
Pollen was extracted from the sediment cores using two different approaches, namely density separation using sodium polytungstate and acetolysis, followed by FTIR microscopy imaging.
Thus, in addition to testing the feasibility of the FTIR imaging approach on different sediment types, we were were also able to investigate how the IR spectra of the *Pinus* pollen vary as a result of two different extraction procedures.
...
knitr::kable(sites, booktabs = T, format = "latex", 
             caption = "Sampled sediment cores for this study. ",
...
First, *Pinus* spp. is a shade-intolerant, high pollen producer, and so a number of studies have proposed that *Pinus* spp. pollen may be a useful taxon to use as a biological indicator for reconstructing past changes of UV-B radiation [@willisQuantificationUVFlux2011; @jardineProxyReconstructionUltravioletB2020; @seddonPollenchemistryVariationsElevation2021].
Second, although prominent in Quaternary sediment records from Europe, and despite the highest number of species amoing conifers [110 species, @gernandtPhylogenyClassificationPinus2005], *Pinus* is currently mostly only split into two different pollen types using fossil pollen, reflecting the two phylogenetic lineages [@beugLeitfadenPollenbestimmungFur2004; @gieseckePostglacialChangeFloristic2019].
...
Note, that there is no agreement on the density of the pollen exine, with reported values varying between <1.4 g cm^-3^ [@flenley1971measurements; @faegriTextbookPollenAnalysis1989; @regnellPreparativeCentrifugationNew1996], 1.88 cm^-3^ [@nakagawaDensemediaSeparationMore1998] and 2.2 cm^-3^ [@juvigneDensiteExinesQuelques1973]. 
...
Preprocessing with SG and EMSC has been shown to improve the performance of multivariate analyses, such as principal component analysis (PCA) or models in the partial least squares regression (PLSR) family [@zimmermannOptimizingSavitzkyGolayParameters2013; @woutersenNovelApproachStudy2018].

We used two PLSR models to explore the chemical differences between modern and fossil grains (model 1), and acetolysed and non-acetolysed fossil grains (model 2).
...
```{r ordinations, echo=FALSE, fig.cap="a. PLSR ordination scores of all samples showing the difference between fresh vs fossil pollen b. regression coefficient of component 1 c. regression coefficient of component 2; d PLSR ordination scores using only samples from sediment cores. Difference between non-acetolysed and acetolyses; e. regression coefficient component 1; f. regression coefficient component 2", message=FALSE, warning=FALSE, out.width="100%"}
knitr::include_graphics(here("figures", "ord_plots.png"))
```

\newpage
## Chemical differences between pollen samples
...
For example, acetolysed pollen showed two dominant peaks in the carbohydrate region at 1190 - 1170 and 1040 cm^-1^, which are not visible in the modern spectra or the non acetolysed pollen, and when using the second PLSR model, the acetolysed pollen is clearly separated from the non-acetolysed pollen, with a much greater distance in the ordination space between samples (Fig. \@ref(fig:ordinations) d).
...
```{r SM, echo=FALSE, fig.cap="Image captured from FTIR microscope of sample slide Dalmutladdo 60 cm depth. \\textit{Pinus} pollen is visible as well as debris and dried residue, presumably BSi.", message=FALSE, warning=FALSE, out.width="90%"}
knitr::include_graphics(here("figures", "SM_BSI.png"))
```

In addition to the clear differences between samples, we also observed two potential contamination effects from the sediment core processing.
First, in the Tiefer See non-acetolysed fossil pollen samples (Fig. \@ref(fig:meanspectra)) there was a broad peak at 1630 cm^-1^. 
This 1630 cm^-1^ peak was responsible for part of the separation between Tiefer See non-acetolysed samples from the other cores, but does not change the larger differentiation of acetolysed Tiefer See samples (without the 1630 cm^-1^ peak) from the other cores, and the peak very likely indicates the presence of SPT [see spectra from @SodiumMetatungstateHydrate].
Thus, it is possible that the three washes with water after decanting of the pollen fraction were not enough in this case, but absence of this peak in the other spectra seem to suggest that further washes may remove successfully remove SPT fully.

Second, we also observed a broad peak at 1100 - 1090 cm^-1^ in the non-acetolysed spectra of all three sediment cores (Fig. \@ref(fig:meanspectra)).
...
## Extraction of *Pinus* FTIR spectra using the FPA detector
In this study we investigated the use of an FPA detector in combination with a FTIR microscope that allows capture of several spectra in order to characterise the chemistry of pollen grains from a sediment sample.
...
The FPA-MCT detector on our FTIR microscope allows simultaneous capture of a larger area of the sample (500 x 500 µm ), compared to other detectors [e.g. 100 x 100 µm @jardineChemotaxonomyDomesticatedGrasses2019, 50 x 50 µm @zimmermannChemicalCharacterizationIdentification2018].
...
For example, the spectra obtained from our modern samples are very similar to *Pinus sylvestris* spectra measured using FTIR-bulk sample methodologies [@zimmermannCharacterizationPollenVibrational2010; @bagciogluMultiscaleVibrationalSpectroscopic2015] and FTIR microscopy using single-point measurements [@zimmermannChemicalCharacterizationIdentification2018].
...
Thus, moving forward we see potential in the application of the FPA detector in chemical palaeoecology. 

## Variable effects of acetolysis
Another important result in our study is the large differences in FTIR spectra between acetolysed and non-acetolysed grains.
The presence of the peak in the 1170 cm^-1^ shoulder in the spectra from the acetolysed pollen grains suggests an overall change in the pollen chemistry of the sample.
...
Interestingly, when comparing our acetolysed *Pinus* spectra to other acetolysed fossil *Pinus* pollen, we observed some differences.
...
Conversely, we found greater similarities between the FTIR spectra from the acetolysed *Pinus* pollen from our study to acetolysed pollen grains of other pollen types.
For example, the carbohydrate region at 1200 - 1000 cm^-1^ in our acetolysed *Pinus* pollen samples is most similar to an acetolysed *extant* Poaceae set of @jardineSporopolleninChemistryIts2021, and in addition to acetolyzed herbarium *Nitrariaceae* pollen [@woutersenNovelApproachStudy2018], where two peaks at 1040 and 1190/1170 are visible and similar to our samples.
...
Taken together, the differences in the effects of acetolysis across different taxa and sample types suggest that chemical extraction methods have an effect on the chemistry of pollen, both modern and fossil origin.
Direct comparisons between spectra from different spectrometers are difficult, because peak heights are not always directly comparable, but these results indicate that further research is required to understand the reasons between these differences between the acetolysed *Pinus* pollen from across the sample sets.
...
For example, it would be useful to explore the effects of these extraction methods on modern and fossil material side-by-side, to investigate if there are methods that can bridge the transition from modern to fossil pollen.
...
The chemical effects of acetolysis that identified across studies suggests that it may be preferable to use an alternative approach to extract fossil pollen grains from sediment samples.
Given the limited chemical influence on the fossil pollen grains, we believe that density separation is a viable alternative to acetolysis and other acid or oxidizing extractions [@regnellPreparativeCentrifugationNew1996; @caffreyUseLithiumHeteropolytungstate2013; @leipeTestingPerformanceSodium2019], although given the fact that an SPT peak was observed in one of the cores, the results in this paper clearly demonstrate the need to identify efficient dilution processes that may need to be tailored to specific sediment types.
Similarly, since the reported density of pollen is variable and may be dependent on age of the sample [@flenley1971measurements; @juvigneDensiteExinesQuelques1973; @faegriTextbookPollenAnalysis1989; @regnellPreparativeCentrifugationNew1996; @nakagawaDensemediaSeparationMore1998], it is possible that specific extraction densities and procedures may be required from core to core.

Since our non-acetolysed samples are characterised by a BSi peak in the FTIR spectra, the density-separation approach also appears to introduce an additional important effect.
Although the BSi peak does not obscure any of the sporopollenin peaks in the FTIR spectra, it does obscure the region 1110 - 1070 cm^-1^.
...
## Future developments
In this study we focused on the chemistry of *Pinus* pollen, because of it's importance in studies related the UV-B-related research and due to its high ecological importance in Quaternary sequences, particularly in the northern hemisphere.
*Pinus* pollen is also relatively large, which prevents any scattering problems found with smaller grains and made for easy identification of pollen granis in from the images created in our experimental set up.
However, the quality of the optical images captured by our microscope was much worse than convential LM, which made it difficult to identify smaller pollen grains on the slide from other organic debris, e.g. *Betula*/*Corylus*, *Alnus* or *Quercus*.
An objective with higher magnification (20x) may improve this aspect of using a FTIR microscope.
Since FTIR measurements of small particles (5-25 µm) introduces scattering artifacts into the spectra [@lukacsRecoveryAbsorbanceSpectra2015; @zimmermannVibrationalMicrospectroscopyEnables2015], sample processing and computational methods exist to minimize or avoid scattering artifacts for these smaller taxa using FTIR.

Finally, a potential future area of research would then be to investigate the possibility for automation to streamline the spectral-collection process. 
...
Acetolysed fossil pollen was compared to non-acetolysed fossil pollen, which revealed differences in their chemical composition (e.g. changes in the 1200 - 1000 cm^-2^ region) that are most similar to reported acetolysed spectra from other taxa, suggesting that acetolysis alters fossil pollen chemistry.
We also report contaminations observed in non-acetolysed samples and the applications of high density liquid extraction for FTIR microspectroscopy and palynology.

<!--
## I haven't found a way to integrate these ideas in there. Perhaps we don't need them? I think perhaps we could leave it out
