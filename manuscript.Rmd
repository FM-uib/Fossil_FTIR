...
There is also evidence of environmental signals that are recorded in the chemistry of pollen grains, including temperature [@bagciogluMultiscaleVibrationalSpectroscopic2015], nutrients [@kendelChemicalAnalysisPollen2020; @diehnCombiningChemicalInformation2020],  and UV-B radiation [@rozemaUVBAbsorbanceUVB2001; @blokkerDevelopmentProxySurface2005; @fraserUVBAbsorbingPigments2011; @jardineSheddingLightSporopollenin2017].
Thus, in recent years, the study of pollen-grain chemistry has gained increasing focus in palaeoecological research since it is possible that chemical variations observed in sporopollenin may be used as proxies for past environmental conditions, with a particular focus on using sporopollenin chemistry to reconstruct past changes in UV-B radiation **[ref: here my 2019 paper would be a good one to cite]** and for improving the taxonomic precision available for sediment records [@dellannaPollenDiscriminationClassification2009; @zimmermannAnalysisAllergenicPollen2016].
...
Bulk FTIR techniques, which involve the analysis of large quantities of modern-pollen samples (e.g. 1-2 mg) (e.g. @zimmermannCharacterizationPollenVibrational2010, @lahlaliATRFTIRSpectroscopyReveals2014) **[is it possible to have one very short description here about what these techniques entail- e.g. the resolution of the laser?]**, have the advantage of reducing scattering effects from measurements due to the analysis of large sample quantities.
As a result, FTIR spectra obtained from bulk pollen samples have been used identify chemical differences at the genus, and in some cases, species level [@muthreichChemicalVariationsQuercus2020; @zimmermannInfraredSpectroscopyPollen2014; @depciuchIdentificationBirchPollen2018; @diehnDiscriminationGrassPollen2020].
However, these approaches may be less relevant for fossil sequences since the number of pollen grains analysed vastly exceeds the quantity which is practically available from a sediment-core sample. 
...
The approach has been successfully used to show that chemical differences are found between pollen on genus and species level [@julierChemotaxonomyToolInterpreting2016; @woutersenNovelApproachStudy2018;  @zimmermannChemicalCharacterizationIdentification2018; @jardineChemotaxonomyDomesticatedGrasses2019], and microscopic-FTIR techniques have also been used to observe chemical differences to reconstruct changes in UV-B absorbing compounds in different periods across Earth's history [@fraserUVBAbsorbingPigments2011; @jardinePollenSporesBiological2016; @jardineProxyReconstructionUltravioletB2019; @jardineSheddingLightSporopollenin2017].
**[Does this sentence belong in the next paragraph]**. Some microscopes are equipped with automated stages, where multiple locations on the slide can be targeted in sequence to record spectra.

One of key challenges faced by chemical palynologists is the time taken to extract and analyse fossil-pollen samples.
...
A focal plane array detector (FPA) allows capture of an image and matrix of spectra over the same sample area instead of a single spectra at a specific point/area [ref].
...
When combined with imaging processing technology, it potentially allows for more rapid identification and generation of FTIR spectra of pollen grains from sediment samples, but whilst FTIR spectroscopy using FPA detectors have been applied to pollen samples **in aerobiology [or others, check references]**, there has been limited investigation into their application on fossil-pollen residues taken from sediment cores.
...
Given the large variations in FTIR spectra observed across taxa, and the variety of sediment types even from Holocene sequences in Europe, more studies are needed investigate the potential effects of the chemical preparation procedures used to isolate pollen-grains from different pollen types.

In this study we aimed to explore the use of an FPA detector to extract FTIR spectra from fossil-pollen *Pinus* grains.
We aim to develop a workflow which enables comparison of fossil-pollen grains taken from three sediment-core locations **[TABLE 1]**, and compare the FTIR spectra generated using the FPA detector from both modern and fossil-pollen samples. 
Pollen was extracted from the sediment cores using two different approaches (density separation using sodium polytungstate, and acetolysis) and the fossil pollen samples measured using the combined FTIR microscope with FPA detector were compared with modern-pollen samples.
...
[**here we need Boris to add a few sentences here to give the study more context**]- not sure this is needed now? unless he is not happy with FPA detector section?
<!-- Chemical treatment... -->
...
First, *Pinus* spp. is a shade-intolerant, high pollen producer, and so a number of studies have proposed that *Pinus* spp. pollen may be a useful taxon to use as a biological indicator for reconstructing past changes of UV-B radiation [ref- willis 2011, seddon 2017, seddon 2021].
Second, although prominent in Quaternary sediment records from Europe, and despite it's high species diversity in Europe (**Check number of species**), *Pinus* is currently mostly only split into two different pollen types using fossil pollen, reflecting the two phylogenitic lineages [@beugLeitfadenPollenbestimmungFur2004; @gieseckePostglacialChangeFloristic2019].
...
Since SPT is a chemically inert salt, it was expected that, after washingm this treatment should have minimal chemical effect on the chemical spectra inferred from the fossil-pollen grains.
...
Thus, incorporating the acetolysis step on this analysis allows us to compare the effects of pollen-grain spectra from both treated and untreated pollen grains.
...
These chemical differences are also clearly demonstrated in the results of the PLSR model (Fig \@ref(fig:ordinations) a).
...
**The main important discussion point to add here is to note the main differences that you see in the fossil and modern Pine pollen spectra- taken using bulk analyses, and using micro-FTIR. Then you address the issue of comparing the FPA detector. I think the story si that the fresh spectra you have here are quite similar to spectra in other studies, correct?**
...
For example, acetolysed pollen showed two dominant peaks in the carbohydrate region at 1040 and 1170 - 1190 cm^-1^, which are not visible in the modern spectra or the non acetolysed pollen.
This change in the 1170 - 1190 cm^-1^ region is unique to acetolysized pollenand had higher regression coefficients that the surrounding wavenumbers (Fig. \@ref(fig:ordinations)).
The 1040 cm^-1^ peak also had high importance for the PLSR model differentiating acetolysed and non acetolysed pollen (Fig. \@ref(fig:ordinations) e and f).

The presence of this new peak in the shoulder of the 1170 cm^-1^ wavenumbers in the acetolysised pollen grains can be indicative of sporopollenin and carbohydrates, and suggests an overall change in the pollen chemistry of the sample.
The presence of these two dominant peaks may be related to the removal and or alteration of grain-wall carbohydrates (cellulose etc), as well as changes to chemical composition of sporopollenin due to the acetolysis treatment. [FM: this would be answered by acetolysis of the fresh grains!!]
Since the 1190  cm^-1^ peak is not visible in any of the samples from the  non-acetolysed pollen, this likely indicates that the acetolysis treatment is creating this peak.
...
Samples from Tiefer See are more separate from samples of the other two cores, which show more similar scores in the ordination.
...
Since it is likely that the main contributions to the pollen some in the TSK core is *Pinus sylvestris*, it is likely that the same species is represented by all three cores, and the main difference between of the TSK pollen is age.
The TSK samples are from the past 80 years, while MFM covers the YD/Holocene transition and DAL samples are from throughout the Holocene, with the youngest sample several 100 years older than TSK pollen.
...
**[one thing to think about- could it be that the SPT residues on TSK were actually influencing the acetolysis effect in some way?]**

In general there are similarities between the FTIR spectra from the acetolysed *Pinus* pollen from our study and in the acetolysed pollen grains of other taxa
For example, the carbohydrate region at 1000 - 1200 cm^-1^ our acetolysised *Pinus* pollen samples is most similar to the acetolysed extant Poaceae set of @jardineSporopolleninChemistryIts2021 and acetolyzed herbarium *Nitrariaceae* pollen [@woutersenNovelApproachStudy2018], where two peaks at 1040 and 1190/1170 are visible and similar to our samples.
However, spectra from a modern Poaceae data set (sampled from the the Amazon Fan and Western Amazon) [@jardineSporopolleninChemistryIts2021] is quite different in composition to both our fossil pollen, and any extant acetolysed pollen  samples, in which the large peak at 1600 and 1190 peak are not clearly visible.
These fossil Poaceae were treated differently than our pollen and the extant Poaceae set, a combination of heavy liquid separation, Na~4~P~2~O~7~ and lignite as opposed to mainly acetolysis.
...
For example, we observed a broad peak at 1090-1100 cm^-1^ in the non acetolysed spectra of all three sediment cores (Fig. \@ref(fig:meanspectra) b, c and d).
In addition, samples from Tiefer See also shows peak at 1630 cm^-1^, but these peaks are not present in either modern- or in the acetolysed-pollen samples.

Comparisons of FTIR spectra from biogenic-silica spectra from diatoms [@alipourInfraredMicrospectroscopyBionanomaterials2016] and hydrated silica spectra [@SilicaATRIRSpectrum] reveal that the peak 1090 cm^-1^ in the non acetolysed samples is most likely biogenic silica, which may have precipitated onto the onto ZnSe slides as the sediment residues being prepared for analysis. 
During the measurements with the microscope, we applied our extracted pollen as droplets onto ZnSe slides and waited for the water to evaporate and we observed a crystalline layer over the same area that covered all particles and pollen **(ref Foto in SM)**.
Interestinglyl, it appeared that the acetolysis treatment removed this peak, which may be because of the extended washing with several chemicals and water during standard acetolysis.
**[Could we say something here like- for pollen chemistry research this may present a challenge if we are aiming to directly compare spectra from unprocessed pollen samples since it may create differences in the spectra beween modern and fossil. I also dont know if it could interfere with the other sporopollenin peaks. A solution would be to apply some baseline correction procedures on this peak first, whihc may be important since even if we found it in these core, the quantity of biogenic silica may vary between different sediment cores from different lakes and regions. This is something to be explored]**
...
**[what about the 1630 peak- unclear if this is a different peak or something else?]**

A similar contamination effect was observed in the Tiefer See samples (Figure XX). This is SPT. Need to add in discussion about importance of cleaning- maybe the sediment type meant more washing was required for this specific sediment type?

In addition to these potential contamination effects, some of the largest differences in fossil pollen samples were the results chemical differences between acetolysised and non-acetolysised samples.

**If this section is discussing differences between fossil vs fresh pollen then the next three paragraphs don;t fit. They are more discussing acetolysised and non acetolysed differences. As result the discussion feels like it is wandering, Therefore, I moved the differences between fossil pollen treatments up**

## Conclusions/ Applications for palaeoecological research
...
The approach has the potential to be an improvement on previous single-grain measurements using FTIR, where each grain is required to be targeted individually, or even manually extracted from the sediment matrix using a micro-pippette.
