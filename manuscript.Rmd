...
One of the most common methods for sporopollenin extraction involves an acetolysis treatment, with which a combination with HF and HCl treatments enables removal of organic materials, calcium carbonates and silicates, while preserving the morphological integrity of pollen grains [@erdtmanSixNotesPollen1959; @faegriTextbookPollenAnalysis1989].
...
The regression coefficients reflect our observations from the visual inspection and show that lipid  and protein peaks (1745 and 1800 - 1500 cm^-1^) are no longer present and there are also substantial changes in the carbohydrate region (1250 - 1000 cm^-1^) in the fossil pollen (Fig \@ref(fig:ordinations) b and c).
The region 1200 - 1000 cm^-1^ is now dominated in acetolysed fossil pollen by two peaks and its shoulders at 1170 and 1040 cm^-1^.

Our modern spectra are very similar to *Pinus sylvestris* spectra measured using FTIR-bulk sample methodologies [@bagciogluMultiscaleVibrationalSpectroscopic2015; @zimmermannCharacterizationPollenVibrational2010] and FTIR microscopy using single point measurements [@zimmermannChemicalCharacterizationIdentification2018].
The same characteristic peaks described above are visible in all modern spectra captured with bulk and FTIR microscopy, e.g. 1745, 1515, 1170 and 1250 - 1000 cm^-1^.
...
This change in the 1190 - 1170 cm^-1^ region is unique to acetolysized pollenand had higher regression coefficients that the surrounding wavenumbers (Fig. \@ref(fig:ordinations)).
...
Since the 1190 cm^-1^ peak is not visible in any of the samples from the  non-acetolysed pollen, this likely indicates that the acetolysis treatment is creating this peak in the FTIR spectra.
...
The TSK samples are from the past 80 years, while MFM covers the YD/Holocene transition and DAL samples are from throughout the Holocene, with the youngest sample more than 100 years older than TSK pollen.
...
@jardineSporopolleninChemistryIts2021 have shown chemical differences between pollen on much larger timescales, where modern pollen had more in common with quarternary pollen than deep time samples.
Our results also indicate chemical difference with age, but on a much shorter timescale.
...
If we compare our *Pinus* spectra to other acetolysised fossil *Pinus* pollen, we observed some differences.
@jardineProxyReconstructionUltravioletB2020 extracted *Pinus* pollen from a varved sediment core using acetolysis and HF to study the UV-B response in *Pinus* pollen chemistry.
Their spectra are similar to ours in the 1800 - 1500 cm^-1^ region, a prominent peak at 1515 cm^-1^ and peaks at 1700 and 1600 cm^-1^.
The region between 1500 - 1300 cm^-1^ is fairly flat in our spectra, while a strong peak is visible at 1450 in @jardineProxyReconstructionUltravioletB2020 spectra.
...
Direct comparisons between spectra from different spectrometers are difficult, because peak heights are not always directly comparable.
Nevertheless, we do see substantial differences in the 1500 - 1000 cm^-1^ region, while there are similarities in the 1800 - 1500 cm^-1^ region

Additionally, there are similarities between the FTIR spectra from the acetolysed *Pinus* pollen from our study to the acetolysed pollen grains of other taxa.
For example, the carbohydrate region at 1200 - 1000 cm^-1^ in our acetolysised *Pinus* pollen samples is most similar to the acetolysed extant Poaceae set of @jardineSporopolleninChemistryIts2021 and acetolyzed herbarium *Nitrariaceae* pollen [@woutersenNovelApproachStudy2018], where two peaks at 1040 and 1190/1170 are visible and similar to our samples.
...
These fossil Poaceae were treated differently than our pollen and the extant Poaceae set, a combination of density separation, Na~4~P~2~O~7~ and lignite as opposed to mainly acetolysis.

Taken together, the simililarities of the acetolysed extant set to our acetolysed fossil set and dissimilarities to pollen treated with a different combination of extractions (Amazon Western and Fan), suggest that chemical extraction methods have a large effect on the chemistry of pollen, both modern and fossil origin.
...
Further studies, in which the effects of these extraction methods need to be explored on modern and fossil material side by side to see if there are methods that can bridge the transition from modern to fossil pollen.
It seems from the comparison of our spectra to other published fossil spectra, that chemical treatments have a large effect on pollen chemistry and may affect fossil and modern grains differently.
...
In addition, samples from Tiefer See also shows a peak at 1630 cm^-1^, but these peaks are not present in either modern- or in the acetolysed-pollen samples.
...
An additional contamination effect was observed in the Tiefer See samples (Fig. \@ref(fig:meanspectra)) with the peak at peak at 1630 cm^-1^. 
...
Acetolysis is the conventional method to extract pollen in high concentration in Quarternary pollen samples, but the results presented here demonstrate the value in extracting non-acetolysised pollen from sediments, because of the substantial differences in pollen chemistry we found between acetolysised and non-acetolysised pollen.
We believe that density separation is a viable alternative to acetolysis and other acid or oxidizing extractions [@caffreyUseLithiumHeteropolytungstate2013; @leipeTestingPerformanceSodium2019; @regnellPreparativeCentrifugationNew1996].
The reported density of pollen is variable and may be dependent on age of the sample [@faegriTextbookPollenAnalysis1989; @flenley1971measurements; @regnellPreparativeCentrifugationNew1996; @nakagawaDensemediaSeparationMore1998; @juvigneDensiteExinesQuelques1973].
...
In summary, fossil pollen from three sediment cores and modern material was analysed by FTIR microspectroscopy using a new detector that allows simultaneous capture of multiple single grain spectra.
