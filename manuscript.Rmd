...
As a result, FTIR spectra obtained from bulk pollen samples have been used to identify chemical differences at the genus, and in some cases, species and population level [@muthreichChemicalVariationsQuercus2020; @zimmermannInfraredSpectroscopyPollen2014; @depciuchIdentificationBirchPollen2018; @diehnDiscriminationGrassPollen2020; @bagciogluMonitoringPlantEnvironment2017; @zimmermannHighthroughputFTIRSpectroscopy2017].
...
However, FTIR microspectroscopy is hindered by strong Mie scattering of IR radiation on pollen grains, resulting with anomalous features in their FTIR spectra [@zimmermannVibrationalMicrospectroscopyEnables2015; @blumelObservationMieRipples2018a; @lukacsRecoveryAbsorbanceSpectra2015].
Although the scattering issues can be tackled by a number of analytical and numerical solutions [@diehnDiscriminationGrassPollen2020; @zimmermannChemicalCharacterizationIdentification2018; @lukacsRecoveryAbsorbanceSpectra2015; @zimmermannAnalysisAllergenicPollen2016], FTIR measurement of individual pollen grains remains challenging.
...
Measurements are usually acquired by consecutive recording of spectra by a single element detector [@zimmermannAnalysisAllergenicPollen2016;@zimmermannChemicalCharacterizationIdentification2018; @zimmermannVibrationalMicrospectroscopyEnables2015].
When spectra are recorded with equidistant spatial steps (i.e. scanning measurement), the individual spectra can be assembled after the recording to obtain a chemical image [@jardineSporopolleninChemistryIts2021; @zimmermannVibrationalMicrospectroscopyEnables2015; @julierChemotaxonomyToolInterpreting2016].
...
Taken together, the differences in the effects of acetolysis across different taxa and sample types suggest that the chemical extraction methods have a large effect on the chemistry of pollen, both modern and fossil origin.
...
Given the limited chemical influence on the fossil pollen grains, we believe that density separation is a viable alternative to acetolysis and other acid or oxidizing extractions [@caffreyUseLithiumHeteropolytungstate2013; @leipeTestingPerformanceSodium2019; @regnellPreparativeCentrifugationNew1996], although given the fact that an SPT peak was observed in one of the cores, the results in this paper clearly demonstrate the need to identify efficient dilution processes that may need to be tailored to specific sediment types.
