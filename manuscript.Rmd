...
Bulk FTIR techniques, which involve the analysis of large quantities of modern-pollen samples (e.g. 1-2 mg) (e.g. @zimmermannCharacterizationPollenVibrational2010, @lahlaliATRFTIRSpectroscopyReveals2014) **[is it possible to have one very short description here about what these techniques entail- e.g. the resolution of the laser? FM: spectra resolution is the same as micro afaik, but signal to noise is better]**, have the advantage of reducing scattering effects from measurements due to the analysis of large sample quantities, improving signal to noise compared to microscopes.
...
We aim to develop a workflow which enables comparison of fossil-pollen grains taken from three sediment-core locations (Table \@ref(tab:sites)), and compare the FTIR spectra generated using the FPA detector from both modern and fossil-pollen samples. 
...
Our modern spectra are very similar to *Pinus sylvestris* spectra measured using bulk sample [@bagciogluMultiscaleVibrationalSpectroscopic2015; @zimmermannCharacterizationPollenVibrational2010] and FTIR microscopy using single point measurements [@zimmermannChemicalCharacterizationIdentification2018].
The same characteristic peaks described above are visible in all modern spectra captured with bulk and FTIR microscopy, e.g. 1745, 1515, 1170 and 1000 to 1250 cm^-1^.
**The main important discussion point to add here is to note the main differences that you see in the fossil and modern Pine pollen spectra- taken using bulk analyses, and using micro-FTIR. FM: I dont know of any bulk fossil spectra of Pinus published. Then you address the issue of comparing the FPA detector. I think the story si that the fresh spectra you have here are quite similar to spectra in other studies, correct?**
...
**[one thing to think about- could it be that the SPT residues on TSK were actually influencing the acetolysis effect in some way? FM: I think it is unlikely. The first step of acetolysis is acetic acid wash, which removes most of the water. I would assume that it removes most if not all of the spt there before the heavy acid step.]**
...
For example, we observed a broad peak at 1090-1100 cm^-1^ in the non acetolysed spectra of all three sediment cores (Fig. \@ref(fig:meanspectra)).
...
For pollen chemistry research this may present a challenge if we are aiming to directly compare spectra from unprocessed pollen samples since it may create differences in the spectra beween modern and fossil.
Fortunately, the BSi peak does not cover any of the sporopollenin peaks, but obscures the region 1070 to 1110 cm^-1^.
The quantity of biogenic silica may vary between different sediment cores from different lakes and regions, which may influence the severety of the interference caused by BSi.
A solution would be to apply correction procedures during preprocessing, or explore extendend washing iterations after extraction of the pollen to dilute BSi.

A similar contamination effect was observed in the Tiefer See samples (Fig. \@ref(fig:meanspectra)) with the peak at peak at 1630 cm^-1^. 
This peak belongs very likely to SPT, when compared to this sodium metatungstate spectra [@SodiumMetatungstateHydrate].
...
This 1630 cm^-1^ peak was responsible for part of the separation between Tiefer See non acetolysed samples from the other cores, but does not change the larger differentiation of acetolysed Tiefer See samples (without the 1630 cm^-1^ peak) from the other cores.

In addition to these potential contamination effects, some of the largest differences in fossil pollen samples were the results of chemical differences between acetolysised and non-acetolysised samples.
